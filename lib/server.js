...
const theDealer = require('../models/oauth-MD.js');
...
// sign up route that takes name and pass then checkAndSave them at the DB .
...
// redirected route for github Oauth
app.get('/oauth', theDealer , afterOauth);
function afterOauth(req , res){
  res.status(200).send(`Nice Work Stranger !!! ${req.token}`);
}

// secret router to test transporterAuth middleware 
app.get('/secret' , transporterAuth , (req , res) => {
  let userName = req.userName;
  if(userName){
    res.status(200).send(`Open Sesame ==> ${userName}`);
  }else{
    res.send('ターゲットを見つけることができません ==> You did Something Wrong ');
  }
...
app.get('/readonly' , transporterAuth , protector('read') , (req , res) => {
  res.status(200).send('Read Only !');
});

app.get('/create' , transporterAuth , protector('create') , (req , res) => {
  res.status(200).send('Create Only !');
});

app.get('/update' , transporterAuth , protector('update') , (req , res) => {
  res.status(200).send('You can Update !!');
});

app.get('/delete' , transporterAuth , protector('update') , (req , res) => {
  res.status(200).send('You Can Delete !');
});

app.get('/everything' , transporterAuth , protector('superuser') , (req , res) => {
  res.status(200).send('Welcome Sensei!!');
});
